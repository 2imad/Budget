   ...
   return {
      init: function () {
         ...
         UIctl.displayBudget({
            ...
            totalExp: data && data.totals.exp,
            totalInc: data && data.totals.inc
         });
         data && data.allItems.exp.forEach(item => UIctl.addListItem(item, 'exp'))
         data && data.allItems.inc.forEach(item => UIctl.addListItem(item, 'inc'))
         setupEventListener();
      }
   };
